	...
	class WLM_INTEGRATION_AUTHORIZENET_ARB {
		...
		public function authorizenet_arb( $that ) {
			...
			if ( ! in_array( $action, $valid_actions ) ) {
				_e( "Invalid Action.", "wishlist-member" );
				die();
			}
		...
		private function purchase_direct() {
			...
			if ( ! isset( $subscriptions[ $id ] ) ) {
				_e( "Invalid Transaction ID. Transaction was not processed.", "wishlist-member" );
				die();
			}

			$nonce = isset( $_POST['nonce'] ) ? $_POST['nonce'] : '';
			if ( ! wp_verify_nonce( $nonce, "purchase-direct-{$id}" ) ) {
				_e( "Permission Denied. Transaction was not processed.", "wishlist-member" );
				die();
			}
