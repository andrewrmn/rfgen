	...
	private function _auth() {
		...
		switch ( $this->method ) {
			case 'GET':
				...
				return $response;
			case 'POST':
				if ( !trim( $data['key'] ) || !in_array( $data['key'], $this->valid_hashes() ) ) {
				...
				foreach ( (array) $this->data['Users'] as $uid ) {
					if ( $wpdb->get_var( $wpdb->prepare( "SELECT `ID` FROM {$wpdb->users} WHERE `ID`=%d", $uid ) ) ) {
						...
						$api_data = array(
							...
							'SendMailPerLevel' => (bool) wlm_arrval( $this->data, 'SendMail' ),
							'ObeyLevelsAdditionalSettings' => (bool) wlm_arrval( $this->data, 'SendMail' ),
							'ObeyRegistrationRequirements' => (bool) wlm_arrval( $this->data, 'SendMail' )
						);
						wlmapi_update_member( $uid, $api_data );
			...
			case 'PUT':
				if ( $wpdb->get_var( $wpdb->prepare( "SELECT `ID` FROM {$wpdb->users} WHERE `ID`=%d", $member_id ) ) ) {
					...
					if ( $this->data['ObeyLevelsAdditionalSettings']  && isset( $this->wpm_levels[ $level_id ]) ) {
						//For Expired Members
						$expired      = $WishListMemberInstance->LevelExpired( $level_id, $member_id );
						$resetexpired = $wpm_levels[ $level_id ]['registrationdatereset'] == 1;
						if ( $expired && $resetexpired ) {
							$WishListMemberInstance->UserLevelTimestamp( $member_id, $level_id, time() );
						} else {
							...
							$resetactive = $wpm_levels[ $level_id ]['registrationdateresetactive'] == 1;
							if ( $levelexpires && $resetactive && !$level_is_ongoing) {
								...
								if ( $levelexpires_cal && $levelexpire_regdate ) {
									...
									$WishListMemberInstance->UserLevelTimestamp( $member_id, $level_id, mktime( $xhour, $xminute, $xsecond, $xmonth, $xday, $xyear ) );
								}
							}
						}
					}
		...
		switch ( $this->method ) {
				...
				if ( empty( $member_id ) ) {
					...
					if( is_array( $this->data['return_fields'] ) ) {
						foreach( $this->data['return_fields'] AS $fld ) {
							$user->UserInfo->$fld = $user->UserInfo->$fld;
						}
					}
			...
			case 'PUT':
				...
				if ( ! isset( $data['display_name'] ) ) {
					$data['display_name'] = trim( wlm_arrval( $data, 'first_name' ) . ' ' . wlm_arrval( $data, 'last_name' ) );
				...
				if( 'POST' == $this->method ) {
					...
					$nickname = trim( wlm_arrval( $data, 'nickname' ) );
					if( !$nickname ) {
						// get nickname from first_name, display_name or user_login in that order
						// also strip @ and everything that from it
						$nickname = preg_replace( '/@.*$/', '', wlm_arrval( $data, 'first_name' ) ?: ( wlm_arrval( $data, 'display_name' ) ?: wlm_arrval( $data, 'user_login' ) ) );
					}

					// check if nickname is unique and generate a unique one (by appending an incremental numnber) if it's not
					$regex = sprintf( '^%s[0-9]*$', $wpdb->escape( $nickname ) );
					$nicknames = $wpdb->get_col( $x = "SELECT `meta_value` FROM `{$wpdb->usermeta}` WHERE `user_id` <> {$data["ID"]} AND `meta_key`='nickname' AND `meta_value` REGEXP '{$regex}'" );
					$ctr = '';
					while( in_array( $nickname . $ctr, $nicknames ) ) {
						$ctr++;
					}
				...
				if ( $this->data['ObeyRegistrationRequirements'] ) {
					...
					foreach ( $nlevels as $key => $_nlevel ) {
						...
						$is_level_ppp = $WishListMemberInstance->IsPPPLevel( $_nlevel );
						if( $is_level_ppp ) {
							$_lname                 = trim( $is_level_ppp->post_title );
							$nlevels[ $key ]        = $_lname;
							$new_levels[ $_nlevel ] = $_lname;
							$all_levels[ $_nlevel ] = $_lname;
							continue;
					...
					if ( ! empty( $confirm_levels ) ) {
						if ( $this->data['SendMailPerLevel'] ) {
							...
							foreach ( $confirm_levels as $lkey => $email_macros['[memberlevel]'] ) {
								...
								$email_macros['[confirmurl]'] = get_bloginfo( 'url' ) . '/index.php?wlmconfirm=' . $member_id . '/' . md5( $user->user_email . '__' . $user->user_login . '__' . $lkey . '__' . $WishListMemberInstance->GetAPIKey() );
