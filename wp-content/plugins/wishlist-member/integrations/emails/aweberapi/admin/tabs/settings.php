...
<div class="-integration-keys">
	<div class="row api-required">
		<template class="wlm3-form-group">
			{
				label : '<?php _e( 'Your AWeber API Authorization Key', 'wishlist-member' ); ?>',
				type : 'textarea',
				...
				column : 'col-md-9',
				readonly : 'readonly',
				tooltip : '<?php _e( 'Use this same key if you wish to connect your other WishList Member sites to AWeber.', 'wishlist-member' ); ?>'
			}
		</template>
		<div class="col-md-3">
			<label>&nbsp;</label>
			<a class="btn btn-block -default -condensed -no-icon save-keys"><span class="-processing"><?php _e( 'Processing...', 'wishlist-member' ); ?></span><span class="-connected"><?php _e( 'Disconnect', 'wishlist-member' ); ?></span><span class="-disconnected"><?php _e( 'Connect to AWeber', 'wishlist-member' ); ?></span></a>
		</div>
	</div>
	<div class="api-disconnected">
		<div class="row">
			...
			<div class="col-md-3">
				<div class="switch-toggle switch-toggle-wlm" id="hassites-toggle">
					...
					<label for="-hassites-yes"><?php _e( 'Yes', 'wishlist-member' ); ?></label>
		...
		<div id="hassites" class="tab-content">
			<div class="row tab-pane" id="hassites-yes">
				<template class="wlm3-form-group">
					{
						label : '<?php _e( 'Paste your existing AWeber API Authorization Key below and click "Connect to AWeber"', 'wishlist-member' ); ?>',
						type : 'textarea',
						name : 'auth_key',
						rows : 4,
						help_block : '<?php _e( 'The existing AWeber API Authorization Key can be obtained from any WishList Member site that is already connected to AWeber. You must be running WishList Member 2.91.3174 or higher in order to copy the key from that site. Please note all WishList Member sites that connect to the AWeber API must use the same AWeber API Authorization Key.', 'wishlist-member' ); ?>',
						column : 'col-md-9'
					}
				</template>
				<div class="col-md-3">
					<a class="btn btn-block -default -condensed save-keys"><span class="-processing"><?php _e( 'Processing...', 'wishlist-member' ); ?></span><span class="-connected"><?php _e( 'Disconnect', 'wishlist-member' ); ?></span><span class="-disconnected"><?php _e( 'Connect to AWeber', 'wishlist-member' ); ?></span></a>
				</div>
			</div>
			<div class="row tab-pane active" id="hassites-no">
				<div class="col-md-12">
					<p><?php _e( 'Use the link below to access a page that will prompt you to enter your AWeber login information and then click Allow Access.', 'wishlist-member' ); ?></p>
					<a class="btn -primary -condensed" href="https://auth.aweber.com/1.0/oauth/authorize_app/2d8307c8?oauth_callback=<?php echo urlencode($callback); ?>" target="_parent">
						<span><?php _e( 'Connect WishList Member to AWeber', 'wishlist-member' ); ?></span>
					</a>
