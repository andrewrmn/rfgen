	...
	<div style="-moz-border-radius: 6px;
     ...
     padding: 8px 8px;">
		<small class="form-text text-muted">
				<b><?php _e( 'Additional Info on Recurring Billing:', 'wishlist-member' ); ?></b><br><br>
				<?php _e( 'The integration charges a one cent ($0.01) payment to the card when a subscription is created in order to ensure the credit card is valid. That payment is immediately refunded once the card is validated successfully.', 'wishlist-member' ); ?><br><br>
				<?php _e( 'Please Note: Creating a subscription transaction successfully does not guarantee subscription payments will process through your Authorize.net account successfully as Authorize.net processes all the actual payments at a set time every day.', 'wishlist-member' ); ?><br><br>
				<?php _e( 'All the details on this integration can be found in this ', 'wishlist-member' ); ?>
...
<div
	...
	<div class="body">
			...
			<template class="wlm3-form-group">
				{
					...
					tooltip : '<?php _e( 'One Time Payment Amount', 'wishlist-member' ); ?>',
				}
			</template>
			<template class="wlm3-form-group">
				{
					...
					tooltip : '<?php _e( 'Recurring Amount', 'wishlist-member' ); ?>', 
				}
			</template>
			<template class="wlm3-form-group">
				{
		...
		<div class="row -anetarb-recurring">
			<template class="wlm3-form-group">
				{
					label : '<?php _e( '<span class="text-nowrap">Charge Every</span>', 'wishlist-member' ); ?>',
			...
			<template class="wlm3-form-group">
				{
					label : '<?php _e( 'Cycles', 'wishlist-member' ); ?>',
					name : 'anetarbsubscriptions[<%- id %>][recur_billing_cycle]',
					column: 'col-3',
					type : 'number',
					min : '0',
					placeholder : '<?php _e( 'Unlimited', 'wishlist-member'); ?>',
					tooltip : '<?php _e( 'Leave blank or set to 0 for Unlimited cycles', 'wishlist-member' ); ?>',
				}
			</template>
		</div>
		<div class="row -anetarb-recurring">
			<template class="wlm3-form-group">
				{
					...
					tooltip : '<?php _e( 'Leave blank or set to 0 to disable trial period', 'wishlist-member' ); ?>',
				}
			</template>
			<template class="wlm3-form-group">
				{
