...
if ( !class_exists('WLM_OTHER_INTEGRATION_eLearnCommerce') ) {
	...
	class WLM_OTHER_INTEGRATION_eLearnCommerce {
		...
		function wpep_addon_wish_list_register( $addon_controller ) {
			global $WishListMemberInstance;

			add_action( 'init', array( $this, 'wpep_remove_scrip_conflicts' ), 50 );

			if( !class_exists('WPEPAddOnWishList') ) {
				require_once( $WishListMemberInstance->pluginDir3 . '/integrations/others/elearncommerce/wpep-inc/controller.php' );
			...
			if ( !is_admin() ) return;
			if ( WPEPAddOnWishList::instance()->has_requirements_met() !== true ) return;

			if ( !class_exists( 'WPEPAddOnWishListSettings' ) ) {
				require_once( $WishListMemberInstance->pluginDir3 . '/integrations/others/elearncommerce/wpep-inc/settings.php' );
			}

			$addon_controller->register_settings_page( WPEPAddOnWishListSettings::instance() );
		}

		function wpep_remove_scrip_conflicts() {
			if( !is_admin() ) return;
			if ( isset( $_GET['page'] ) && $_GET['page'] == 'WishListMember' ) {
