<?php
	...
	$activate_thirdparty_providers = (array) $this->GetOption('ActiveShoppingCarts');
?>
<div id="all-integrations-parent" class="show-saving">
			...
			<div class="row integration-providers pt-3">
				...
				<div class="col-md-2 col-sm-3 col-xs-4 -providers" data-name="<?php echo $integration_name; ?>">
					<div id="thirdparty-provider-<?php echo $config['id']; ?>" class="integration-toggle-container text-center<?php echo $active; echo $no_settings; ?>">
						<a href="<?php echo add_query_arg(['page' => $this->MenuID, 'wl' => 'setup/integrations/payment_provider/' . $config['id']], admin_url('admin.php')); ?>" class="integration-toggle <?php echo $config['id']; ?>" data-provider="<?php echo $config['id']; ?>" data-title="<?php echo $config['name']; ?>">
						...
						<h5 class="title-label"><?php echo $integration_name; ?></h5>
					</div>
				</div>
				<?php
				endforeach;
	...
	<div id="wlm3-thirdparty-provider">
		<?php foreach($thirdparty_providers AS $provider) :
			if($provider != basename($requested_integration)) continue;

			$path = $this->pluginDir3.'/integrations/payments/' . $provider;
			...
			?>
			<div id="thirdparty-provider-container-<?php echo $provider; ?>" data-parent="#all-integrations-parent" data-type="payment" data-link="<?php echo wlm_arrval($config, 'link'); ?>" data-name="<?php echo wlm_arrval($config, 'name'); ?>" data-provider="<?php echo $provider; ?>" class="thirdparty-provider-container collapse">
...
<?php
	...
	echo "var thirdparty_provider_index_format = 'integration.shoppingcart.%s.php';\n";
	echo "</script>\n";
?>