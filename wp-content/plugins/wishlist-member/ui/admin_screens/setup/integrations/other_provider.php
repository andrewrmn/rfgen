<?php
	...
	$webinar_data = $this->GetOption('webinar');

	if($webinar_provider) {

		$activate_thirdparty_providers[] = $webinar_provider;

		$this->DeleteOption('WebinarProvider');

		$this->SaveOption('active_other_integrations', array_unique($activate_thirdparty_providers));
	}

	$_old = $this->GetOption('ActiveIntegrations');
	if(is_array($_old) && count($_old)) {
		...
		$this->SaveOption('active_other_integrations', array_unique($activate_thirdparty_providers));
		$this->DeleteOption('ActiveIntegrations');
	}
?>
	...
	<div class="content-wrapper collapse <?php if($requested_integration == '*') echo 'show'; ?>" data-parent="#all-integrations-parent" id="all-integrations">
		<form action="">
			<div class="row integration-providers pt-3">
				...
				?>
				<div class="col-md-2 col-sm-3 col-xs-4 -filtered <?php echo $config['group']; ?> -providers" data-name="<?php echo $integration_name; ?>">
					<div id="thirdparty-provider-<?php echo $config['id']; ?>" class="integration-toggle-container text-center<?php echo $active; echo $no_settings; ?>">
						<a href="<?php echo add_query_arg(['page' => $this->MenuID, 'wl' => 'setup/integrations/other_provider/' . $config['id']], admin_url('admin.php')); ?>" class="integration-toggle <?php echo $config['id']; ?>" data-provider="<?php echo $config['id']; ?>" data-title="<?php echo $config['name']; ?>">
	...
	<div id="wlm3-thirdparty-provider">
		<?php
			...
			foreach($thirdparty_providers AS $provider) :
				...
				$path = $this->pluginDir3.'/integrations/others/' . $provider;
			...
			?>
			<div id="thirdparty-provider-container-<?php echo $provider; ?>" data-parent="#all-integrations-parent" data-type="other" data-link="<?php echo wlm_arrval($config, 'link'); ?>" data-name="<?php echo wlm_arrval($config, 'name'); ?>" data-provider="<?php echo $provider; ?>" class="thirdparty-provider-container collapse">
