	...
	<div id="legacy_reg_fields">
		<div class="row">
			<template class="wlm3-form-group">{
				...
				type : 'textarea',
				column: 'col-md-12',
				label: '<?php _e('New Member Registration > Full Instructions', 'wishlist-member'); ?>',
			}</template>
		</div>
		<div class="row mb-5">
		...
		<div class="row">
			<template class="wlm3-form-group">{
				...
				type : 'textarea',
				column: 'col-md-12',
				label: '<?php _e('New Member Registration > Instructions if "Existing Users Link" is Disabled', 'wishlist-member'); ?>',
			}</template>			
		</div>
		<div class="row mb-5">
			<div class="col-sm-5">
		...
		<div class="row">
			<template class="wlm3-form-group">{
				...
				type : 'textarea',
				column: 'col-md-12',
				label: '<?php _e('Existing Member Registration > Full Instructions', 'wishlist-member'); ?>',
			}</template>			
		</div>
		<div class="row mb-5">
			<div class="col-sm-5">
...
<script type="text/javascript">
	var $reset_data = {
		reg_instructions_new : <?php echo json_encode( $WishListMemberInitialData[ 'reg_instructions_new' ] ); ?>,
		reg_instructions_new_noexisting : <?php echo json_encode( $WishListMemberInitialData[ 'reg_instructions_new_noexisting' ] ); ?>,
		reg_instructions_existing : <?php echo json_encode( $WishListMemberInitialData[ 'reg_instructions_existing' ] ); ?>
	}
	var $data = {
		reg_instructions_new : <?php echo json_encode( $this->GetOption( 'reg_instructions_new' ) ); ?>,
		reg_instructions_new_noexisting : <?php echo json_encode( $this->GetOption( 'reg_instructions_new_noexisting' ) ); ?>,
		reg_instructions_existing : <?php echo json_encode( $this->GetOption( 'reg_instructions_existing' ) ); ?>
	}
	$(function() {
