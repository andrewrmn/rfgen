...
class OW_Process_Flow {
    ...
    */
	...
	public function validate_submit_to_workflow() {
		...
		$is_bypass_warning = isset( $_POST['by_pass_warning'] ) ? intval( $_POST['by_pass_warning'] ) : "";
    ...
    */
	...
	public function submit_post_to_step() {
		...
		$sign_off_comments = isset( $_POST["sign_off_comments"] ) ? $this->sanitize_comments( nl2br( $_POST["sign_off_comments"] ) ) : "";
	...
	public function reassign_process( $reassign_data = null ) {
		...
		if ( $data["assign_actor_id"] != - 1 ) { // assignment or publish step (reassigned)
			...
			if ( $is_api == true ) {
				return array(
					"isError"         => "false",
					"successResponse" => esc_html__( "The post was successfully reassigned." )
				);
			} else {
				wp_send_json_success();
			}
		...
		if ( $is_api == true ) {
			return array(
				"isError"       => "true",
				"errorResponse" => esc_html__( "The post can not be reassigned." )
			);
		} else {
			wp_send_json_error();
		}
    ...
    */
	...
	public function workflow_cancel( $api_data = null ) {
		...
		if ( $new_action_history_id ) {
			...
			$msg
				= sprintf( esc_html__( '<div>Hello %1$s,</div><p>The post <a href="%2$s" title="%3$s">%3$s</a> has been cancelled from the workflow.</p>',
				'oasisworkflow' ),
    ...
    */
	...
	public function enqueue_acf_validator_script() {
		...
		if ( ! empty( $acf_version ) && $acf_version >= "5.0.0" &&
		     $acf_version <= "5.6.9" ) { // applicable to pro and free version > 5.x of ACF
			wp_enqueue_script( 'owf_acf_validator',
		...
		if ( ! empty( $acf_version ) &&
		     $acf_version == "5.6.10" ) { // applicable to pro and free version = 5.6.10 of ACF
			wp_enqueue_script( 'owf_acf_validator',
				OASISWF_URL . 'js/pages/acf-pro-validator.js',
				array( 'jquery' ), OASISWF_VERSION, true );
		}
	...
	public function check_is_role_applicable( $post_id ) {
		...
		$owf_is_applicable_filter = apply_filters( 'owf_is_applicable', $post_id );
	...
	public function elementor_submit_to_workflow() {
		...
		// sanitize_text_field remove line-breaks so do not sanitize it.
		$comments = ( ( isset( $_POST["comment"] ) ) ? $this->sanitize_comments( nl2br( $_POST["comment"] ) ) : '' );
	...
	public function workflow_submit_action( $location, $post_id ) {
		if ( isset( $_POST["save_action"] ) &&
		     sanitize_text_field( $_POST["save_action"] ) == "submit_post_to_workflow" ) {
	...
	public function submit_post_to_workflow() {
		...
		/* sanitize incoming data */
		$step_id = isset( $_POST["hi_step_id"] ) ? intval( $_POST["hi_step_id"] ) : "";
		...
		$publish_date = isset( $_POST["hi_publish_datetime"] )
			? sanitize_text_field( $_POST["hi_publish_datetime"] ) : "";
