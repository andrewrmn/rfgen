...
class OW_History_Service {
	...
	public function delete_history( $period = "" ) {
		...
		$posts_not_in_workflow_results = $wpdb->get_results( $wpdb->prepare(
			"SELECT distinct post_id FROM " . $wpdb->fc_action_history . " WHERE post_id IN
		(SELECT ID from
		(SELECT posts.ID from " . $wpdb->posts . " AS posts WHERE 1=1 %s"  . " AND posts.id NOT IN " . "(SELECT A.post_id FROM
		...
		ON A.ID = B.action_history_id)) as wp_posts_temp )", $range) );
			...
			if ( $histories ):
				...
				foreach ( $histories as $key => $row ) {
					if ( $row->assign_actor_id != - 1 ) { //assignment and/or publish steps
                        ...
                        $comments = apply_filters( 'ow_comments_history_report', $comments, $row->ID );
					...
					if ( has_filter( "owf_download_history_column_content" ) ) {
						$data[ $i ] = apply_filters( "owf_download_history_column_content", $row->post_id,
							$data[ $i ] );
					}
