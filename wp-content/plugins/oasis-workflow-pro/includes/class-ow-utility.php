...
class OW_Utility {
	...
	public function get_priority_dropdown( $sel_priority = '2normal' ) {
		...
		$option     = '';
		foreach ( $priorities as $key => $val ) {
			$option .= '<option value="' . esc_attr( $key ) . '" ' . esc_html__( selected( $sel_priority, $key, false ) ) . '>' .
	...
	public function get_purge_history_period_dropdown( $selected_period = 'one-month-ago' ) {
		...
		foreach ( $period as $key => $val ) {
			$option .= '<option value="' . esc_attr( $key ) . '" ' . esc_html__( selected( $selected_period, $key, false ) ) . '>' .
			           esc_html__( $val, 'oasisworkflow' ) . '</option>';
		}
		echo $option;
	}
