...
define( 'OASISWF_VERSION', '8.4' );
define( 'OASISWF_DB_VERSION', '8.4' );
...
class OW_Plugin_Init {
	...
	private static function delete_for_site() {
		...
		$wpdb->query( "DROP TABLE IF EXISTS " . $wpdb->fc_emails );
		$wpdb->query( "DROP TABLE IF EXISTS " . $wpdb->fc_action_history );
		$wpdb->query( "DROP TABLE IF EXISTS " . $wpdb->fc_action );
		$wpdb->query( "DROP TABLE IF EXISTS " . $wpdb->fc_workflow_steps );
		$wpdb->query( "DROP TABLE IF EXISTS " . $wpdb->fc_workflows );
	}
	...
	private function get_current_screen_pointers() {
		...
		$welcome_message_2
		                   = sprintf( esc_html__( "1. Activate the plugin by providing a valid license key on Workflows %s Settings, License tab.",
			"oasisworkflow" ), $img_html );
		$welcome_message_3
		                   = esc_html__( "2. Create a new workflow OR modify/use the sample workflows that come with the plugin.",
			"oasisworkflow" );
		$welcome_message_4
		                   = sprintf( esc_html__( "3. Activate the workflow process by going to Workflows %s Settings, Workflow tab.",
			"oasisworkflow" ), $img_html );
		if ( function_exists( 'is_multisite' ) && is_multisite() ) {
